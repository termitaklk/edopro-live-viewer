...
def recibir_mensajes(sock, text_area_bin, text_area_hex, msg_window_bin, msg_window_hex):
    try:
        ...
        while True:
            ...
            mensaje_bin = f"Mensaje {count} ({message_size} bytes) = {response}"
            mensaje_hex = f"Mensaje {count} ({message_size} bytes) = {response.hex()}"

            mensajes_binarios.append([f"Mensaje {count} ({message_size} bytes)", response])
            mensajes_hexadecimales.append([f"Mensaje {count} ({message_size} bytes)", response.hex()])
...
def main(room_id_hex, server_ip):
    ...
    print('Codigo hexa', create_room_hex)
...
def seleccionar_archivo():
    ruta_bin = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")], title="Guardar mensajes binarios")
    ruta_hex = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")], title="Guardar mensajes hexadecimales")
...
treeview = ttk.Treeview(root, columns=("Room ID", "Users", "Notes", "Start LP"), show="headings")
for room in rooms:
